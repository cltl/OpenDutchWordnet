...
class Synsets():
    ...
    def synsets_add_synset(self,
        ...
        if sy_id not in self.ili_dict:
            raise ValueError('no ili identifier found for {}'.format(sy_id))

        ili = self.ili_dict[sy_id]

        # check if sy_id already exists
        if sy_id in self.syn_ids:
            raise ValueError('synset exists already: {}'.format(sy_id))
...
<MonolingualExternalRefs/>
        ...
        for reltype, target in rels:
            ...
            if reltype == 'has_hyperonym':
                added_hypernym_rel = True

        if added_hypernym_rel or sy_id.endswith('a'):
